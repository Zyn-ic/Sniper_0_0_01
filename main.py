...
try:
 ...
 class Sniper:
    ...
    def __init__(self) -> None:
        ...
        self.sniper = "Off"
        self.token = self.config['blankstuff']['token']
        self.Free_Limited = self.config['blankstuff']['free_limited']
        self.Payed_Limited = self.config['blankstuff']['paid_limited']
        self.Time = time.time()
        ...
        #self.auto_id_updated()
        #threading.Thread(target=self.auto_id_updated, daemon=False).start()
        #print("STARTED AUTO ID UPDATED CHECKER"), os.system("pause")
    ...
    def _load_items(self) -> list:
        ...
        for id in ids:
            ...
            id_dictionary[id] = {
            'current_buys': 0,
            'max_buys':  float('inf') if self.config["items"]['item_list'][0]['max_buys'] is None else int(self.config["items"]['item_list'][0]['max_buys']),
            'max_price': float('inf') if self.config["items"]['item_list'][0]['max_price'] is None else int(self.config["items"]['item_list'][0]['max_price']),
        }
            print(id_dictionary)
            #os.system("pause")
            ...
            'embeds': [
                {
                    ...
                    'title': embed[0],
                    'fields': [
                        {'name': 'Price', 'value': embed[1]},
                        {'name': 'ID', 'value': embed[2]}
                    ]
                }
            ]
        }       

        response = requests.post(self.config['webhook']['url'], json=payload)
        if response.status_code == 204:
            print("Message sent successfully!")
        else:
            print("Failed to send message.")

    def getfromdis(self, channelid):
        headers = {
        ...
        # if you want to snipe more than one channel
        if len(channelid) > 1:
            for i in channelid:
                if i not in ob['ids']:
                    ob['ids'][i] = {'checker': None, 'r': None, 'jsonn': None}
                ...
                if ob['ids'][i]['jsonn'] and ob['ids'][i]['jsonn'][0]['content'] != ob['ids'][i]['checker']:
                    ...
                    if self.config['webhook']['enabled']:
                        ...
                        b = ob['ids'][i]['jsonn'][0]['embeds'][0]['fields'][1]['value']
                        c = self.return_id(ob['ids'][i]['jsonn'][0]['embeds'][0]['url'])
                        d = ob['ids'][i]['jsonn'][0]['embeds'][0]['url']
                        self.send_discord_message([a,b,c, d])

                    id.append(self.return_id(ob['ids'][i]['jsonn'][0]['embeds'][0]['url']))
        else:
            # if you ONLY wanted to snipe one channel [faster]
            channel_id = channelid[0]
            if channel_id not in ob['ids']:
                ob['ids'][channel_id] = {'checker': None, 'r': None, 'jsonn': None}
            ...
            if ob['jsonn'] and ob['jsonn'][0]['content'] != ob['ids'][channel_id]['checker']:
                ...
                if self.config['webhook']['enabled']:
                    ...
                    c = self.return_id(ob['jsonn'][0]['embeds'][0]['url'])
                    d = ob['jsonn'][0]['embeds'][0]['url']
                    self.send_discord_message([a,b,c, d])
                #print(f"name: {ob['jsonn'][0]['embeds'][0]['title']}")
                #print(f"price: {ob['jsonn'][0]['embeds'][0]['fields'][1]['value']}")
                #print(f"id: {self.return_id(ob['jsonn'][0]['embeds'][0]['url'])}", '\n-------------------------------')
                id.append(self.return_id(ob['jsonn'][0]['embeds'][0]['url']))

        return id

     
    
    def _retrived_auto_search_ids(self):
        logging.info("AUTO SEARCH HAS RUNNING")
        if self.config['blankstuff']['use_auto_search'] == True:
                self.sniper = "Running..."
                self.task = "Item Scraper & Searcher WITH AUTO SEARCH"
                    
                e = None
                passing = []
                if self.config['blankstuff']['free_limited'] == True:
                    passing.append(1094291863332192376)
                if self.config['blankstuff']['paid_limited'] == True:
                    passing.append(1093342484358250576)
                if len(passing) == 0:
                    self.sniper = "WARNING YOU ARE NOT SNIPPING ANYTHING ||TURN ON (Paid or Free) LIMITED IN CONFIG||"
                    return

                e = self.getfromdis(passing)
            ...
            self.sniper = "Sleeping..."


    """ Idea one: I'm going to have 2 methods, [1 new and 1 old] or [2 new and 1 old]
      just doing to reuse _load_items but not load from file.
      put the ids to the back always [nul][nul][id][id] 
      """
    # END OF AUTO SEARCH
    ################################################################################################################################
    ...
    def auto_id_updated(self):
        while self.config['blankstuff']['real_time_id_update']:
            if (time.time() - self.Time) >= 10:   
                ...
                self._load_items()
                if self.sniper == "Sleeping":
                    for i in ob["ids"]:
                        print(len(ob['ids']), "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                        self.items[self.return_id( i['jsonn'][0]['embed'][0]['url'] )]  = {
                            'current_buys': 0,
                            'max_buys':  float('inf') if self.config["items"]['item_list'][0]['max_buys'] is None else int(self.config["items"]['item_list'][0]['max_buys']),
                            'max_price': float('inf') if self.config["items"]['item_list'][0]['max_price'] is None else int(self.config["items"]['item_list'][0]['max_price']),
                        }
                    ...
                    for i in ob["ids"]:
                        ...
                        self.items[self.return_id( i['jsonn'][1]['embed'][0]['url'] )]  = {
                                'current_buys': 0,
                                'max_buys':  float('inf') if self.config["items"]['item_list'][0]['max_buys'] is None else int(self.config["items"]['item_list'][0]['max_buys']),
                                'max_price': float('inf') if self.config["items"]['item_list'][0]['max_price'] is None else int(self.config["items"]['item_list'][0]['max_price']),
                        }
     ...
     async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=None), timeout=aiohttp.ClientTimeout(total=None)) as session:
      ...
      while True:
        try:
                    ...
                    #print("running auto_S")
                    self._retrived_auto_search_ids()
